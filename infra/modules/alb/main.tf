...
resource "aws_security_group" "alb" {
  ...
  description = "ALB ingress on 80/443 from internet (locked down by WAF), egress to ECS tasks."
  vpc_id      = var.vpc_id
...
resource "aws_security_group" "ecs" {
  ...
  description = "ECS tasks accept traffic from the ALB only; egress everywhere."
  vpc_id      = var.vpc_id
...
# v1: HTTP-only listener while the ACM cert / DNS is being negotiated.
# Phase 8 swaps this for an HTTPS listener using `var.certificate_arn`.

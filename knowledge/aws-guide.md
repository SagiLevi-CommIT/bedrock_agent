...
The CardiacSense data agent accesses AWS services in the **staging account** (735555370207) in **eu-central-1** (Frankfurt). All access is read-only.
...
3. Profile name: `cardiac-sense-staging-s3`
...
# CLI profile name: cardiac-sense-staging-s3
```
...
```bash
aws sso login --profile cardiac-sense-staging-s3
```

### Verify Access

```bash
aws sts get-caller-identity --profile cardiac-sense-staging-s3
```
...
|----------|---------|-------------|
| `AWS_PROFILE` | `cardiac-sense-staging-s3` | AWS profile for all data access |
...
```bash
# List buckets
aws s3 ls --profile cardiac-sense-staging-s3

# List a prefix
aws s3 ls s3://735555370207-migrated--data/time-series-data/ --profile cardiac-sense-staging-s3

# List with details
aws s3 ls s3://735555370207-app-events/rearrangement/rt_flow/ --profile cardiac-sense-staging-s3 --recursive --summarize
```
...
```bash
# List databases
aws glue get-databases --profile cardiac-sense-staging-s3

# List tables
aws glue get-tables --database-name migrated_data --profile cardiac-sense-staging-s3

# Describe a table
aws glue get-table --database-name migrated_data --name timeseries --profile cardiac-sense-staging-s3
```
...
```bash
# List dashboards
aws quicksight list-dashboards --aws-account-id 735555370207 --profile cardiac-sense-staging-s3

# List datasets
aws quicksight list-data-sets --aws-account-id 735555370207 --profile cardiac-sense-staging-s3
```
...
Ensure `AWS_PROFILE=cardiac-sense-staging-s3` is set, or pass `--profile` explicitly.
